...
Plot3   <- ggplot(plotdat, aes(Year, Average, colour = Type)) + 
        geom_line() +
        ggtitle("Total Emissions in Baltimore by Type")#+ facet_grid(rows = vars(Type)) ##Facets for multiple graphs if preferred
